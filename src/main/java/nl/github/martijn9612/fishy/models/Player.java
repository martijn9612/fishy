...
public class Player extends Entity {
    ...
    private static final String[] BITE_SOUNDS = { MusicPlayer.BITE_SOUND_1,
            MusicPlayer.BITE_SOUND_2, MusicPlayer.BITE_SOUND_3 };
     ...
     * 
     * @param dimensions
     *            size of the new player.
     * @param position
     *            vector with the start position of the player.
     * @param velocity
     *            initial speed of the player.
     * @param acceleration
     *            initial acceleration of the player.
     * @param loadResources
     *            whether the sprite resources should be loaded.
     */
    public Player(Vector dimensions, Vector position, Vector velocity,
            Vector acceleration, boolean loadResources) {
    ...
    public static Player createPlayer(boolean loadResources) {
        ...
        return new Player(dimensions, position, velocity, acceleration,
                loadResources);
    }
     ...
     * 
     * The player will be moved realistically in an environment of water. Also
     * the user input on the keyboard is handled here.
     * 
     * @param gc
     *            the container holding the game.
     * @param deltaTime
     *            time elapsed since method was called in milliseconds.
     */
    ...
    private void fillHashMap() {
        images.put("none", PLAYER_SPRITE);
        images.put("half", PLAYER_HALF_SHIELD_SPRITE);
        images.put("full", PLAYER_FULL_SHIELD_SPRITE);
    }
    ...
    private void movePlayer(Input input) {
        boolean moveL = (input.isKeyDown(Input.KEY_A) || input
                .isKeyDown(Input.KEY_LEFT));
        boolean moveR = (input.isKeyDown(Input.KEY_D) || input
                .isKeyDown(Input.KEY_RIGHT));
        boolean moveU = (input.isKeyDown(Input.KEY_W) || input
                .isKeyDown(Input.KEY_UP));
        boolean moveD = (input.isKeyDown(Input.KEY_S) || input
    ...
    public void eat(double opponentSize) {
        ...
        float newDimension = PLAYER_WIDTH
                + Math.round(score * PLAYER_EAT_GROW_FACTOR);
        dimensions = new Vector(newDimension, newDimension);
        Main.actionLogger.logLine("Player ate opponent", getClass()
                .getSimpleName());
        Main.actionLogger.logLine("Player score is " + Math.floor(score),
    ...
    public void playBiteSound() {
        int biteSoundNumber = (int) Math.ceil(BITE_SOUNDS.length
                * Math.random()); /* Integer between 1 and array length */
